...
def radar_const(name):
    ...
    m = re.search(rf"^{re.escape(name)}\s*=\s*(-?\d+)", open("radar.s").read(), re.M)
    if not m:
...
art = Image.open("../../assets/png/radar100.png").convert("RGBA")
aw, ah = art.size
ap = art.load()
