...
; =============================================================================
; THE GAME LIVES HERE. This file, and the src/ around it, is a byte-for-byte
; copy of proto/03_radar — the bench that answered the radar — promoted whole
; and now the only living code in the repo. proto/ is frozen: it is the record
; of what each question cost, not something to edit or fork again. If you are
; changing behaviour, you are changing THIS file.
;
; What that inheritance means in practice: everything here has been measured,
; and some of it is still bench scaffolding rather than game. The scaffolding
; is deliberate and stays until the questions it serves are answered —
;   * the HUD readout (HUD_ON) and the ladders behind FIRE and joystick 2:
;     open_questions B1 and B3 are still open and are decided by flying them.
;   * ROCK_FAMILY, the opcode switch: a measurement control.
;   * landmark.s, marked TEMPORARY, and the reduced outlines left unused in
;     shapes.s (design_technical 11.9).
; None of it is load-bearing for the game, and each carries its own note.
;
...
;   * NOBJ drifting asteroids with real world positions, velocities and spins,
;     drawn as SOLID FULL-RES outlines ($4E POLYGON16 — the family is a switch,
;     see ROCK_FAMILY) at one of five sizes: 192, 128, 64, 32 and 16 full-res
;     pixels across. They collide with each other; see physics.s.
;   * the RADAR, hard into the bottom-left corner of the portrait screen:
;     every enemy within 25,600 world units, and every rock of the two largest
;     size classes that still exist, one point each, on a scale that does not
;     move with the zoom. Enemies blink. Its ring and ship icon are a BACKGROUND
...
;     not read as one more star. See radar.s — it is what this bench is for.
;   * a HUD reading out speed, turn rate, the visible star count and how many
;     rocks and contacts were drawn. HUD_ON = 1 here - see the note
;     on it, and on why this bench wants it where proto 02 did not.
;
...
;   FIRE           cycle the turn rate — the knob proto 01 was here to judge
;
...
; for nothing (5.5).
;
; It is OFF, as it was in proto 02, and the radar no longer needs it: the
; instrument suppresses the starfield inside its own disc (add_radar_occluder),
; so it reads as a dark hole with contacts in it rather than as a patch of sky
; that happens to have extra stars. Turn it on to read the counters - the STARS
; line carries the contact count in its R field - at 83.9% of CPU1's worst
; frame against 78.5%, and a derived outline budget of 78 vertices against 104.
HUD_ON      = 0
